...
export class ExerciseMediaRepository {
  ...
  static async ensureExerciseMediaDir() {
    const dir = mediaDir();
  ...
  static getExerciseMediaDirectory(): string {
    return mediaDir();
  }

  static getFileExtension(uri: string, fileName: string | null | undefined, fallback: PickerMediaType): string {
    const name = fileName ?? '';
  ...
  static getMediaForExercise(exerciseId: string): ExerciseMedia[] {
    try {
  ...
  static async addMedia(exerciseId: string, media: PickerMedia): Promise<ExerciseMedia | null> {
    try {
      await this.ensureExerciseMediaDir();
      const id = generateId();
      const ext = this.getFileExtension(media.uri, media.fileName, media.type);
      const destUri = `${this.getExerciseMediaDirectory()}${id}.${ext}`;
  ...
  static async replaceMedia(
    ...
    try {
      ...
      await this.ensureExerciseMediaDir();
      const ext = this.getFileExtension(newPickerUri, newFileName, media.media_type);
      const destUri = `${this.getExerciseMediaDirectory()}${mediaId}_new.${ext}`;
